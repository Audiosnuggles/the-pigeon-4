...
export function initAudio(tracks, updateRoutingFromUI) { 
    ...
    // Delay Setup
    fxNodes.delay.node.delayTime.value = 0.4; fxNodes.delay.feedback.gain.value = 0.4;
    fxNodes.delay.in.connect(fxNodes.delay.node); fxNodes.delay.node.connect(fxNodes.delay.feedback); 
    fxNodes.delay.feedback.connect(fxNodes.delay.node); fxNodes.delay.node.connect(masterGain);

    // Reverb Setup
    fxNodes.reverb.mix.gain.value = 0.3;
    fxNodes.reverb.in.connect(fxNodes.reverb.node); fxNodes.reverb.node.connect(fxNodes.reverb.mix); 
    fxNodes.reverb.mix.connect(masterGain);

    // Vibrato Setup
    fxNodes.vibrato.node.delayTime.value = 0.005; fxNodes.vibrato.lfo.frequency.value = 5;
    fxNodes.vibrato.lfo.connect(fxNodes.vibrato.depthNode); fxNodes.vibrato.depthNode.connect(fxNodes.vibrato.node.delayTime);
    fxNodes.vibrato.lfo.start(); fxNodes.vibrato.in.connect(fxNodes.vibrato.node); 
    fxNodes.vibrato.node.connect(masterGain);

    tracks.forEach((t, i) => {
        ['delay', 'reverb', 'vibrato'].forEach(fx => {
            let send = audioCtx.createGain(); send.gain.value = 0;
            send.connect(fxNodes[fx].in); trackSends[i][fx] = send;
        });
    });
